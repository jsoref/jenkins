    ...
    @Override protected void before() throws Exception {
        ...
        Future<Channel> f1 = executors.submit(new Callable<Channel>() {
            public Channel call() throws Exception {
                return new ChannelBuilder("This side of the channel", executors).build(p1i, p2o);
            }
        });
        Future<Channel> f2 = executors.submit(new Callable<Channel>() {
            public Channel call() throws Exception {
                return new ChannelBuilder("The other side of the channel", executors).build(p2i, p1o);
            }
        });
        french = f1.get();
        british = f2.get();
